...
_NOSTZ	CPX #ABY	; handle exceptions
	BEQ _CHABY
	CPX #IMM
	BNE _CONT
	...
	BNE _NOBIT
	ADC #ADJBIT
_NOBIT	CPY #$28	; immediate mode need to adjust a range
	BMI _CONT
	CPY #$2F+1
	BCS _CONT
	ADC #ADJIMM	; carry is clear
;	BNE _CONT
_CHABY	CPY #$35	; LDX check
	BNE _CONT
	CLC
	ADC #ADJABY
_CONT	CPY #$23	; STZ needs special handling
	BNE _DONE
	CPX #ABS
	BMI _DONE
	BEQ _SKIP
	ADC #<($1-$10)	; carry is set
_SKIP	ADC #$30-1	; carry is set
_DONE	JSR DOBYTE	; we have the object code
	.if BRKAS2
	CMP #$00
	BNE _MKARG
	JSR DOBYTE
...
ADDMOD			; Check the arguments and work out the
	...
	CPX #SP
	BNE _NOTSP
_EOL	CMP #$12	; check exception first - JSR
	BEQ _RET
	LDX #IMP	; implied mode - space
	JSR CHKMOD	; check command is ok with this mode
	CPX #FAIL	; not ok
	BEQ _NOTIMP	; may still be accumulator mode though
_RET	RTS
_NOTIMP LDX #ACC	; accumulator mode - space
...
_ERR	LDY #$FC	; Substitute $FC for invalid op,
...
_DOC3	LDA TEMP2
	...
	ADC #$10
	RTS
_NOTC3	LDX TEMP1	; does this code end in 10010?
	TXA
	AND #$1F
	CMP #$12
	BNE _1
	DEX
_1	TXA		; ? ABCD EFGH - thanks bogax, www.6502.org/forum
	ASL		; A BCDE FGH0
	ADC #$80	; B ?CDE FGHA
	ROL		; ? CDEF GHAB
	ASL		; C DEFG HAB0
...
MSGSZ = $21
MSG	.text "3.1 NESSEW NEK YB 20C56 REDASURK",CR
...
_RET	JMP (PCL)	; Simulate the return so we can more easily manipulate the stack
DOCMD	LDX #$FE
_LOOP	JSR GETCH1
	STA ARGS+2,X
	INX
