...
\texttt{DEBUG} routine at address \texttt{\$FE19} (\texttt{\$FE0A} for the 65C02 version), execution
...
To return to the mini-monitor from the Apple 1 monitor, type \texttt{FE23R} (or \texttt{FE0AR} if you are
running the 65C02 version). Another useful address to remember is the disassembly routine at
\texttt{\$FAD0} (\texttt{\$FAF8} for the 65C02 version), but bear in mind that using this routine involves changing
...
\setstretch{1.08}  % Slightly increases line spacing
...
\begin{itemize}
	...
	\item \krusader 1.3 (December 2007)
		\begin{itemize}
			...
			\item Bundled with the Krusader Toolkit\footnote{\url{http://school.anhb.uwa.edu.au/personalpages/kwessen/apple1/ktk.zip}} -- an integrated editor, terminal and emulator for the Replica 1.
		\end{itemize}
\end{itemize}
