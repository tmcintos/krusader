...
and addressing modes, and fixing a few problems with the original design\footnote{See \url{http://www.6502.org/users/obelisk/65C02/} for a brief introduction to the 65C02.}.  
Although these
...
routine at address \texttt{\$FE14} (\texttt{\$FE03} for the 65C02 version), execution of a \texttt{BRK}
...
\texttt{FE1ER} (or \texttt{FE16R} if you are running the 65C02 version).
Another useful address to remember is
the disassembly routine at \texttt{\$FACA}
(\texttt{\$FAEA} for the 65C02 version),
...
The mini-monitor includes no commands for examining or altering memory.
...
\begin{table}[htb]
	...
	\texttt{\$FE14}$^{*\dagger}$ & Mini-monitor entry & \texttt{\$F0-\$F4} -- Register storage (S,P,Y,X and A)  \tabularnewline
	\texttt{\$FE1E}$^{*\dagger}$ & Mini-monitor re-entry & \texttt{\$F5,\$F6} -- Address of next instruction \tabularnewline
	 & & \texttt{\$0F-\$11} -- Input buffer \tabularnewline
	...
	\hline
	\texttt{\$FEC9}$^{\dagger}$ & Get character \newline & N/A -- Input character returned in \texttt{A} register \tabularnewline
	\texttt{\$FEF5}$^{\dagger}$ & Get character (no echo) & \tabularnewline
	\texttt{\$FEF0}$^{\dagger}$ & Print newline & \tabularnewline
	\hline
	\end{tabular}
	\caption{Important function entry points and related memory locations.}
	\caption*{
	$^{*}$ In the 65C02 version, the mini-monitor addresses are \texttt{\$FE03} and \texttt{\$FE16}, and
	the disassembler address is \texttt{\$7BEA(\$FAEA)}. \\
	$^{\dagger}$ ROM version only.
	}
	\label{tab:memlocs}
\end{table}
...
\begin{itemize}
	...
	\item \krusader\ 1.3 (December 2007)
		\begin{itemize}
			...
			\item Bundled with the \textit{Krusader Toolkit}\footnote{\url{https://thewessens.net/collection/apple1/ktk.zip}} 
